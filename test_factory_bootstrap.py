...
SPEC = importlib.util.spec_from_file_location(
    "factory_bootstrap", "factory_bootstrap.py"
)
...
class FactoryBootstrapChecks(unittest.TestCase):
    def test_lookup_success_and_confirmed_absence(self):
        with mock.patch.object(factory, "_gh", return_value=Result(0)):
            self.assertEqual(
                factory._lookup("acme/factory"), (factory.LOOKUP_EXISTS, "")
            )
        with mock.patch.object(
            factory,
            "_gh",
            return_value=Result(1, "GraphQL: Could not resolve to a Repository"),
        ):
    ...
    def test_failed_lookup_is_indeterminate(self):
        for message in (
            ...
            with (
                ...
                mock.patch.object(factory, "_gh", return_value=Result(1, message)),
            ):
                self.assertEqual(
                    factory._lookup("acme/factory")[0], factory.LOOKUP_INDETERMINATE
                )
    ...
    def test_indeterminate_lookup_blocks_all_creation_even_with_yes(self):
        ...
        with (
            mock.patch.object(factory, "_preflight"),
            mock.patch.object(
                factory, "_gh", side_effect=lambda *args: next(results)
            ) as gh,
        ):
            self.assertEqual(
                factory.ensure("acme", "factory", "private", False, True), 1
            )
        self.assertEqual(
            [call.args[:2] for call in gh.call_args_list],
            [("repo", "view"), ("repo", "view")],
        )
    ...
    def test_plan_requires_org(self):
        ...
        error = io.StringIO()
        with (
            mock.patch.object(sys, "argv", ["factory_bootstrap.py", "--plan"]),
    ...
    def test_consent_creates_confirmed_missing_repositories(self):
        with (
            ...
            self.assertEqual(
                factory.ensure("acme", "factory", "private", False, False), 0
            )
    ...
    def test_idempotent_reruns(self):
        ...
        with (
            mock.patch.object(factory, "_preflight"),
            mock.patch.object(factory, "_gh", side_effect=fake_gh),
        ):
            self.assertEqual(
                factory.ensure("acme", "factory", "private", False, True), 0
            )
            self.assertEqual(
                factory.ensure("acme", "factory", "private", False, True), 0
            )
