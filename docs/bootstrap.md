...
- **Script (`init.py`, Python 3 stdlib):** deterministic and repeatable replacement. It first presents the
  ...
  - `python3 init.py --dry-run` shows changes without writing. Normal initialization consumes the source template's ownership contract during cleanup, removing source-only governance (including the determinism checker), provider/CI inputs, and release E2E/triage assets while retaining generic checkers that do not require source-only files, workflows, and generated outputs; `--no-clean` keeps source-only inputs for verification.
