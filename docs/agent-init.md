...
## Paso 2 — Rellenar los mecánicos
Corré el script con los `kind: mechanical` (incluí `CI_STACKS` y `CI_SYSTEM`):
...
## Paso 3 — Resolver los `kind: judgment`
...
## Paso 5 — Verificar ANTES de la autolimpieza
...
- El script se **autolimpió** (`init.py`, `placeholders.json` y `ci/`); el `ci.yml` compuesto queda.
