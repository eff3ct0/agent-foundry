...
| `node scripts/check-delivery-contract.mjs --self-check`, `scripts/check-delivery-contract.py --approval-self-check`, `scripts/check-factory-layout.py`, `node scripts/check-pr-governance.mjs --self-check`, `node start.mjs --self-check`, `node scripts/sync-github-labels.mjs --self-check` | Deterministic, read-only, offline | Same validation result for unchanged files and catalog; the layout check builds its fixture in a temporary directory | No state |
