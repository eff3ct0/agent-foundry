...
| `scripts/report-bootstrap-failure.mjs` | Network-dependent, deduplicated issue reporting | A release-SHA marker, or a tag/run preparation marker when no SHA resolves, searches open and closed issues; one existing canonical issue receives one marker-bearing comment, otherwise one bug-form issue is created | Uses only issue write permission; no source or disposable repository mutation; a rerun is a no-op after the marker-bearing comment exists |
