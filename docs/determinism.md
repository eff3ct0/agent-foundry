...
| `.github/workflows/governance.yml` and `scripts/check-pr-governance.py` | Network-dependent, read-only validation | Revalidates on each configured PR event; GitHub providers read issue labels, other providers validate native references only | Fix metadata or obtain approval in the bound provider |
