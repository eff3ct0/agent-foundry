...
| PR governance composition | Deterministic, local write, idempotent | The selected task provider produces native linkage instructions in both PR templates; unchanged bytes are not rewritten | Retained templates; restore from VCS |
...
| `node scripts/check-delivery-contract.mjs --self-check`, `node scripts/check-delivery-contract.mjs --approval-self-check`, `scripts/check-delivery-contract.py --approval-self-check`, `scripts/check-factory-layout.py`, `node scripts/check-pr-governance.mjs --self-check`, `node start.mjs --self-check`, `node scripts/sync-github-labels.mjs --self-check` | Deterministic, read-only, offline | Same validation result for unchanged files and catalog; the Node workflow and delivery checks are authoritative, the retained Python approval fixture is a bounded compatibility signal, and the layout check builds its fixture in a temporary directory | No state |
...
| `node scripts/sync-github-labels.mjs --dry-run` and `--self-check` | Deterministic, read-only, offline | Same commands/output for the same catalog | No state |
| `node scripts/sync-github-labels.mjs` and `.github/workflows/sync-labels.yml` | Network-dependent, outward, idempotent | `gh label create --force` creates or updates each catalog label; stale labels are intentionally retained | GitHub labels; restore the catalog and rerun, or delete labels manually with approval |
| `.github/workflows/governance.yml` and `scripts/check-pr-governance.mjs` | Network-dependent, read-only validation | Revalidates on each configured PR event; GitHub providers read issue labels, other providers validate native references only | Fix metadata or obtain approval in the bound provider |
...
| `.github/workflows/bootstrap-e2e.yml` and its Node release helpers | Network-dependent, release-triggered, disposable validation | A published or explicitly supplied tag resolves to one commit SHA; every recipe case tests that SHA in its own private repository; repeated runs use a new run prefix | Creates and deletes only `bootstrap-e2e-<run-id>-<case>` repositories in the configured owner; final cleanup is always attempted and exact-prefix recovery is documented |
| `scripts/report-bootstrap-failure.mjs` | Network-dependent, deduplicated issue reporting | Valid evidence uses configured recipe cases, one consistent `OPENAI_MODEL`, and every non-passed matrix result; a release-SHA marker, or a tag/run preparation marker when no SHA resolves, searches open and closed issues; one existing canonical issue receives one marker-bearing comment, otherwise one bug-form issue is created | Uses only issue write permission; no source or disposable repository mutation; a rerun is a no-op after the marker-bearing comment exists |
| `scripts/triage-bootstrap-failure.mjs` | Network-dependent, advisory failure analysis | A sanitized `bootstrap-e2e-failure/v1` payload is bounded, sent with strict `text.format` JSON Schema and `store: false`, and invalid/unavailable model output becomes a deterministic fallback | Receives only sanitized payload, `OPENAI_MODEL`, and `OPENAI_API_KEY` through an `env -i` boundary; no GitHub token, tools, lifecycle control, or issue mutation; triage artifacts are retained 7 days |
