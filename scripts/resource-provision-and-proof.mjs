...
export const validatePersistedProvisioningProof = ({ serialized, runId, owner, resource, template, releaseSha }) => {
  ...
  releaseSha = typeof releaseSha === "string" ? releaseSha.trim().toLowerCase() : "";
  releaseSha = validate(releaseSha, SHA, "invalid_release_sha", "release SHA must be a full immutable commit");
  ...
  if (!object(proof) || serializeProvisioningProof(proof) !== serialized || !Number.isSafeInteger(proof.repository_id)
      || proof.repository_id <= 0 || typeof proof.default_branch !== "string" || !BRANCH.test(proof.default_branch)) return undefined;
  return Object.entries(expectedProof({ runId, owner, name, template, releaseSha })).every(([key, value]) => proof[key] === value) ? proof : undefined;
};
...
export const provisionAndProve = async ({ mutationClient, proofClient, proofStore, runId, owner, resource, template, releaseSha }) => {
  ...
  let result;
  try {
    result = await mutationClient.createTemplateRepository({ template, owner, name });
  } catch {
  ...
  const proof = await proveProvisionedResource({ client: proofClient, runId, owner, resource, template, releaseSha });
  if (proof.status !== "verified") return proof;
  try {
    await proofStore.write(serializeProvisioningProof(proof));
  } catch {
  ...
  let serialized;
  try {
    serialized = await proofStore.read();
  } catch {
    return recoveryRequired("proof_persistence_failed");
  }
  const persisted = validatePersistedProvisioningProof({ serialized, runId, owner, resource, template, releaseSha });
