...
export const cleanupProvisionedResource = async ({ evidence, artifact: artifactStore, readClient, mutationClient, owner, target, runId }) => {
  ...
  const input = { artifact: artifactStore, owner, target, runId };
  let serialized;
  try {
  ...
  if (eligibility.status === "already-absent") return persist(artifactStore, artifact({ ...input, status: "already-absent" }));
  if (eligibility.status !== "eligible") return recovery(input, eligibility.code);

  let deletion;
  try {
    deletion = await mutationClient.deleteRepository({ owner: eligibility.recovery.owner, name: eligibility.recovery.proof.name });
  } catch {
  ...
  return persist(artifactStore, artifact({ ...input, status: "deleted" }));
};
