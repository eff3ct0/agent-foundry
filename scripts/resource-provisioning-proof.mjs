...
export const proveProvisionedResource = async ({ client, runId, owner, resource, visibility = "private", template, releaseSha: expectedSha }) => {
  ...
  runId = validate(runId, RUN_ID, "invalid_run_id", "run ID must be numeric");
  owner = validate(owner, OWNER, "invalid_owner", "resource owner is invalid");
  const name = canonicalResourceName({ runId, resource });
  visibility = validate(visibility, /^(?:private|public|internal)$/u, "invalid_visibility", "resource visibility is invalid");
  template = validate(template, REPOSITORY, "invalid_template", "template identifier is invalid");
  expectedSha = releaseSha(expectedSha);

  const repository = await read(client, `/repos/${owner}/${name}`);
  if (repository.status !== "ok") return repository;
  const details = repository.payload;
  if (details.full_name !== `${owner}/${name}` || details.name !== name) return rejected("resource_name_mismatch");
  if (!object(details.owner) || details.owner.login !== owner) return rejected("resource_owner_mismatch");
  if (!Number.isSafeInteger(details.id) || details.id <= 0) return rejected("repository_id_mismatch");
  if (details.visibility !== visibility || !VISIBILITIES.has(details.visibility)) return rejected("resource_visibility_mismatch");
  if (!object(details.template_repository) || details.template_repository.full_name !== template) return rejected("resource_template_mismatch");
  ...
  const reference = await read(client, `/repos/${owner}/${name}/git/ref/heads/${encodeURIComponent(branch)}`);
  if (reference.status !== "ok") return reference;
  const target = object(reference.payload.object) ? reference.payload.object : undefined;
  if (!target || target.type !== "commit" || target.sha !== expectedSha) return rejected("resource_release_sha_mismatch");
  return repositoryProof({ runId, owner, name, visibility, template, releaseSha: expectedSha, details, branch });
};
