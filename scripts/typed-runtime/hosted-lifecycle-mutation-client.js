...
const contentLength = (headers) => {
    ...
    if (value === null || value === undefined || value === "")
        return undefined;
    if (!/^\d+$/u.test(value))
        fail("invalid_response", "hosted lifecycle response is invalid");
    return Number(value);
};
...
const mutationResult = async (response, expectedStatus, expectedPayload) => {
    if (!object(response) || typeof response.status !== "number" || !Number.isInteger(response.status))
        return { status: "rejected", code: "invalid_response" };
    if (response.status >= 500 && response.status <= 599)
        return { status: "indeterminate", code: "mutation_indeterminate" };
    if (response.status !== expectedStatus)
        return { status: "rejected", code: "mutation_rejected" };
    try {
        const bytes = await boundedBytes(response);
        if (!expectedPayload)
            return bytes.length === 0 ? { status: "deleted" } : { status: "rejected", code: "invalid_response" };
        const payload = JSON.parse(bytes.toString("utf8"));
        return object(payload) ? { status: "created", payload } : { status: "rejected", code: "invalid_json" };
    }
    catch (error) {
        if (error instanceof HostedLifecycleMutationError)
...
export const createHostedLifecycleMutationClient = ({ transport, token }) => {
    if (typeof transport !== "function")
        fail("invalid_transport", "hosted lifecycle transport is required");
    if (typeof token !== "string" || !token || /[\0-\x1f\x7f]/u.test(token))
        fail("invalid_token", "hosted lifecycle token is invalid");
    const mutate = async ({ method, endpoint, payload, expectedStatus, expectedPayload }) => {
        ...
        if (byteLength(serialized) > MAX_REQUEST_BYTES)
