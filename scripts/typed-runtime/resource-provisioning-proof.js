...
const validate = (value, expression, code, message) => {
    if (typeof value !== "string" || !expression.test(value))
        throw new ResourceProvisioningProofError(code, message);
    return value;
};
const releaseSha = (value) => {
    const sha = typeof value === "string" ? value.trim().toLowerCase() : "";
    if (!SHA.test(sha))
        throw new ResourceProvisioningProofError("invalid_release_sha", "release SHA must be a full immutable commit");
    return sha;
};
const read = async (client, endpoint) => {
    ...
    if (!object(result) || result.status === "indeterminate")
        return indeterminate();
    if (result.status !== "ok" || !object(result.payload))
        return rejected("read_rejected");
    return { status: "ok", payload: result.payload };
};
export const canonicalResourceName = ({ runId, resource }) => {
...
export const serializeProvisioningProof = (proof) => `${JSON.stringify({
...
export const proveProvisionedResource = async ({ client, runId, owner, resource, visibility = "private", template, releaseSha: expectedSha }) => {
    if (!object(client) || typeof client.get !== "function")
        throw new ResourceProvisioningProofError("invalid_client", "hosted lifecycle read client is required");
    ...
    const repository = await read(client, `/repos/${verifiedOwner}/${name}`);
    if (repository.status !== "ok")
        return repository;
    const details = repository.payload;
    if (details.full_name !== `${verifiedOwner}/${name}` || details.name !== name)
        return rejected("resource_name_mismatch");
    if (!object(details.owner) || details.owner.login !== verifiedOwner)
        return rejected("resource_owner_mismatch");
    if (typeof details.id !== "number" || !Number.isSafeInteger(details.id) || details.id <= 0)
        return rejected("repository_id_mismatch");
    if (details.visibility !== verifiedVisibility || !VISIBILITIES.has(verifiedVisibility))
        return rejected("resource_visibility_mismatch");
    if (!object(details.template_repository) || details.template_repository.full_name !== verifiedTemplate)
        return rejected("resource_template_mismatch");
    if (typeof details.default_branch !== "string" || !BRANCH.test(details.default_branch))
        return rejected("resource_branch_mismatch");
    const branch = details.default_branch;
    const reference = await read(client, `/repos/${verifiedOwner}/${name}/git/ref/heads/${encodeURIComponent(branch)}`);
    if (reference.status !== "ok")
        return reference;
    const target = object(reference.payload.object) ? reference.payload.object : undefined;
    if (!target || target.type !== "commit" || target.sha !== verifiedSha)
        return rejected("resource_release_sha_mismatch");
