...
const sectionPresent = (text, section) => {
    ...
    if (labels.has(wanted))
        return true;
    if (wanted === "identity" && ["Contract instance", "Capability", "Provider"].every((name) => field(text, name)))
        return true;
...
const contractFiles = (projectRoot) => {
    ...
    if (fs.existsSync(path.join(projectRoot, "providers"))) {
        for (const directory of Object.values(providerDirectories))
            files.push(path.join(projectRoot, "providers", directory, "_contract.md"));
    }
    if (fs.existsSync(path.join(projectRoot, "ci")))
        files.push(path.join(projectRoot, "ci", "_contract.md"), path.join(projectRoot, "ci", "recipes.json"));
    return files;
};
const providerFiles = async (projectRoot) => {
    ...
    for (const [capability, directory] of Object.entries(providerDirectories)) {
        ...
        for (const entry of await readdir(providerDirectory).catch(() => [])) {
            if (entry.endsWith(".md") && entry !== "_contract.md")
                files.push([capability, path.join(providerDirectory, entry)]);
        }
    }
...
const checkLinks = (target, text, projectRoot, errors) => {
    for (const match of text.matchAll(localLink)) {
        ...
        if (!link || link.startsWith("#") || /^[a-z][a-z0-9+.-]*:/iu.test(link))
            continue;
        if (!fs.existsSync(path.resolve(path.dirname(target), link.split("#", 1)[0])))
            errors.push(`${display(target, projectRoot)} broken link: ${link}`);
    }
};
const checkDocument = (target, text, projectRoot, errors) => {
    ...
    for (const relative of ["providers/task/_contract.md", "providers/secrets/_contract.md", "providers/code-intel/_contract.md", "ci/_contract.md"]) {
        ...
        if (fs.existsSync(contract) && !linked.has(path.resolve(contract)))
            errors.push(`${display(target, projectRoot)} missing contract link: ${relative}`);
    }
};
const checkProvider = (capability, target, contractPath, contractText, text, projectRoot, errors) => {
    ...
    for (const section of headings(contractText))
        if (!sectionPresent(text, section))
            errors.push(`${label} missing contract section: ## ${section}`);
    const instance = text.match(contractInstance);
    if (!instance)
        errors.push(`${label} missing Contract instance link`);
    else if (path.resolve(path.dirname(target), instance[1].split("#", 1)[0]) !== path.resolve(contractPath))
        errors.push(`${label} Contract instance link must resolve to ${display(contractPath, projectRoot)}`);
    const expectedCapability = capability === "code-intel" ? "code-intelligence" : capability;
    if (field(text, "Capability").replaceAll("`", "").toLowerCase() !== expectedCapability)
        errors.push(`${label} must declare Capability: ${expectedCapability}`);
    const provider = field(text, "Provider").replaceAll("`", "");
    if (!provider)
        errors.push(`${label} must declare a Provider`);
    else if (path.basename(target, ".md") !== "custom" && provider.toLowerCase() !== path.basename(target, ".md").toLowerCase())
        errors.push(`${label} Provider must match selectable name '${path.basename(target, ".md")}'`);
    if (capability === "code-intel" && path.basename(target, ".md") === "none") {
        ...
        if (missing.length > 0)
            errors.push(`${label} missing explicit intentional-minimal none contract: ${missing.join(", ")}`);
    }
};
const checkCi = (target, projectRoot, errors) => {
    ...
    let recipes;
    try {
        recipes = JSON.parse(fs.readFileSync(target, "utf8"));
    }
    catch (error) {
        errors.push(`${label} invalid CI recipes: ${error.constructor.name}`);
        return;
    }
    if (!recipes || typeof recipes !== "object" || Array.isArray(recipes) || Object.keys(recipes).length === 0) {
        errors.push(`${label} must contain at least one CI recipe`);
        return;
    }
    for (const [name, job] of Object.entries(recipes).sort(([left], [right]) => left.localeCompare(right))) {
        if (name === "_contract") {
            errors.push(`${label} must not select _contract`);
            continue;
        }
        if (typeof job !== "string") {
            errors.push(`${label} recipe '${name}' must be a YAML job string`);
            continue;
        }
        if (!job.includes(ciRecipeMarker))
            errors.push(`${label} recipe '${name}' missing ${ciRecipeMarker}`);
        if (!new RegExp(`^\\s{2}${name.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}:\\s*$`, "mu").test(job))
            errors.push(`${label} recipe '${name}' must expose its recipe key as the job`);
        for (const required of ["runs-on:", "actions/checkout@", "steps:", "run:"])
            if (!job.includes(required))
                errors.push(`${label} recipe '${name}' missing ${required}`);
    }
};
const inferRoot = (paths, projectRoot) => {
    ...
    for (const target of paths) {
        ...
        if (Object.values(providerDirectories).includes(path.basename(directory)) && path.basename(path.dirname(directory)) === "providers")
...
export const check = async (paths, projectRoot = root) => {
    ...
    if (!defaultPaths)
        effectiveRoot = inferRoot(paths, effectiveRoot);
    ...
    for (const target of selected) {
        if (!fs.existsSync(target))
            errors.push(`required file missing: ${display(target, effectiveRoot)}`);
        else
            present.push(target);
    }
    ...
    for (const [capability, directory] of Object.entries(providerDirectories)) {
        ...
        if (fs.existsSync(target))
            contracts.set(capability, [target, await readFile(target, "utf8")]);
    }
    ...
    for (const [capability, target] of providers) {
        ...
        if (contract && fs.existsSync(target))
            checkProvider(capability, target, contract[0], contract[1], await readFile(target, "utf8"), effectiveRoot, errors);
    }
    if (present.includes(path.join(effectiveRoot, "ci", "recipes.json")))
        checkCi(path.join(effectiveRoot, "ci", "recipes.json"), effectiveRoot, errors);
    return errors;
};
export const delegatedApprovalErrors = (evidence, targetIssue) => {
    ...
    const errors = [];
    if (instruction.source !== "direct-human")
        errors.push("instruction must be direct human input");
    if (instruction.current !== true)
        errors.push("instruction must be current");
    if (instruction.issue !== targetIssue)
        errors.push("instruction must name the exact target issue");
    if (instruction.action !== approvalAction)
        errors.push("instruction must name the exact approval action");
    if (principal.evidence_source !== "target-host")
        errors.push("principal authority must come from the target host");
    if (!allowedPrincipalRoles.includes(principal.role ?? ""))
        errors.push("principal lacks target-host maintainer authority");
    if (instruction.principal !== principal.subject)
        errors.push("instruction principal is not bound to target-host evidence");
    if (actor.subject !== principal.subject)
        errors.push("authenticated actor is not the authorized principal");
    if (!allowedActorCapabilities.includes(actor.capability ?? ""))
        errors.push("actor lacks MAINTAIN or ADMIN capability");
    if (operation.issue !== targetIssue || operation.label !== "status:approved")
        errors.push("operation is not scoped to the exact issue and label");
    if (operation.attempts !== 1)
        errors.push("operation must have exactly one add attempt");
    if (JSON.stringify(operation.sequence) !== JSON.stringify(["add", "readback"]))
        errors.push("readback must immediately follow the one add attempt");
    if (operation.result !== "added")
        errors.push("mutation must succeed with a known added result");
    if (readback.issue !== targetIssue || !readback.labels?.includes("status:approved"))
        errors.push("target-host readback does not confirm the approval");
    return errors;
};
...
const main = async () => {
    if (process.argv.length === 3 && process.argv[2] === "--self-check")
        return selfCheck();
    if (process.argv.length === 3 && process.argv[2] === "--approval-self-check")
        return approvalSelfCheck();
