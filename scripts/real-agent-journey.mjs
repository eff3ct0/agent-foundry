...
export const validatePackageName = (value) => { const result = String(value ?? "").trim(); if (result !== "@eff3ct/agent-foundry") throw new JourneyError("real-agent package is unsupported", "package_invalid"); return result; };
...
export const selfCheck = () => { if (journeyRepository("acme", "123") !== "acme/real-agent-journey-123" || validateRuntime("codex-cli") !== "codex-cli" || validatePackageName("@eff3ct/agent-foundry") !== "@eff3ct/agent-foundry" || componentPath("agent") !== COMPONENTS.agent) throw new Error("real-agent journey contract self-check failed"); process.stdout.write("real-agent journey contract self-check OK\n"); };
