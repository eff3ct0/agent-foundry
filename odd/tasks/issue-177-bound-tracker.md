...
BT-02 and BT-03 are committed local work units; BT-04 has passed offline checks but needs its local work-unit commit before it is a reviewable boundary. Do not split tests from behavior, or use a local checkbox as the native review candidate. Issue-wide closeout still needs provider-confirmed tracker readback, subject to the existing authorization and approval gates.
...
- Child #3 commit identity: pending local work-unit commit; record its SHA in a separate passive evidence edit after committing this verified unit. No push, PR, remote mutation, or merge is authorized for this child.
